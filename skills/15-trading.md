...
commands:
  ...
  - trade journal
version: 35
updated_at: 2026-09-23T11:47:38
---
...
- [2026-09-22 14:22] (ai) 漏斗缺席双机制警示(20260922 江南布衣案例): 已持仓/熟识标的缺席master.csv有两种机制须区分——①数据源缺口(如0921-0922上午 HK cashflow缺失致波司登掉票, 源恢复后自动回归); ②漏斗预排名挤出(0922下午 cashflow已恢复但江南布衣仍缺席: F10仅深挖quotes预排名前200, 小市值低换手标的(换手率0.23)会因预排名变化被挤出F10→无ROE/毛利率→被master.csv漏斗剔除, 属结构性缺席非数据错误). 统一补救路径: ask单股链路(live quote+best-available fundamentals)复核, 勿因缺席L1池直接判定论点变化; 但QMF裁决以池内票为准, 池外标的不构成当日L1基础(江南布衣0922未入选当日最被低估即属此类)
- [2026-09-23 11:47] (ai) 弹药层教训(20260923 康臣案例): 两层时机结构(错杀池+企稳扳机)实战首日即暴露第三层缺口——弹药层. 扳机标的康臣药业QMF全链通过(融合裁决: graham gate+ultimate-buyer三问+扳机确认+南向增持+零红旗), 但每手1000股×14.36=14,360HKD>现金6,827,结构性买不起. 教训: ①部署率86%后剩余现金13.9%不足以应付任何中价股的一手入场费, 扳机亮了子弹不够; ②修正: L4裁决新增可负担性前置检查(board lot×现价 vs 现金), 候选在扳机检查前先过可负担性, 买不起的票不进入扳机观察(或明确标注'扳机已亮但弹药不足'状态); ③季赛视角: 高部署率的代价是丧失对扳机事件的响应能力, 留出'一手弹药'(约5000-15000HKD)是部署纪律的一部分, 不是闲置浪费; ④票数规则再校准: '基本面大师票<2不开仓'的本意是防GSL式单策略量化榜首陷阱(NUTX型: 动量顶端+噪声), 康臣是graham门槛全过+质量分89.2池内最高档+股东回报三问全过的深度价值票, L4融合裁决(非机械交集, 用户风格档案明文)判定其通过, 票数规则在此类情形让位于融合裁决
