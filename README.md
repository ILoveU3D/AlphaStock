...
- **财务三通道**：A 股走东财 datacenter 批量接口（含经营现金流，自动回补上季）；美股走 SEC EDGAR XBRL frames（含经营现金流，利润为剥离一次性处置收益的 recurring 口径）；港股走 F10 逐股深度财务（12 个报告期）。
...
- **单股分析（ask）**：任意名称 / 代码 / ticker（中文亦可）经解析链落到唯一标的；输出实时报价 + 快照基本面 + K 线指标 + **市场内百分位** + 风险旗标 + **四周期适合度剖面**（超短/短/中/长各自打分），`--evidence` 展开全证据表。
...
- **纯 JSON 输出**：全部数据命令支持 `--json`——stdout 即纯 JSON（全精度、NaN→null、无横幅），供程序化消费。
- **新鲜度门槛**：价格敏感命令代码级强制检查快照年龄，过期直接拦截而非输出陈旧数据；自动化管线可用 `--no-check` 显式跳过。
- **原子持久化**：用户档案与交易期状态均为临时文件 + 改名原子写，断电不产生半截 JSON。
- **离线测试**：389 个单元测试全部离线运行（网络访问全部 mock），无 API Key 依赖。
